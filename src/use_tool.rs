...
#[cfg(test)]
mod tests {
    use super::*; // Import items from your core crate (TypeConverter, etc.)
    use std::collections::HashMap;

    #[test]
    fn test_vec_json_array() {
        // Input string in JSON array format.
        let input_str = "[1, 2, 3, 4, 5]";
        // Convert it to Vec<i32> using the TypeConverter implementation.
        let result: Result<Vec<i32>, String> =
            <Vec<i32> as TypeConverter>::convert_from_str(input_str);
        assert_eq!(result.unwrap(), vec![1, 2, 3, 4, 5]);
    }

    #[test]
    fn test_vec_comma_separated() {
        // Input string as comma-separated values.
        let input_str = "1, 2, 3, 4, 5";
        let result: Result<Vec<i32>, String> =
            <Vec<i32> as TypeConverter>::convert_from_str(input_str);
        assert_eq!(result.unwrap(), vec![1, 2, 3, 4, 5]);
    ...
    #[test]
    fn test_hashmap_json_object() {
        // Input string in JSON object format.
        let input_str = r#"{"a": 1, "b": 2, "c": 3}"#;
        type HM = HashMap<String, i32>;
        let result: Result<HM, String> = <HM as TypeConverter>::convert_from_str(input_str);
    ...
    #[test]
    fn test_hashmap_key_value_pairs() {
        // Input string as key-value pairs.
        let input_str = "a:1, b:2, c:3";
        type HM = HashMap<String, i32>;
        let result: Result<HM, String> = <HM as TypeConverter>::convert_from_str(input_str);
